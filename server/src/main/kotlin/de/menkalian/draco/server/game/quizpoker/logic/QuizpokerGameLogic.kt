...
import de.menkalian.draco.server.util.addStringList
...
class QuizpokerGameLogic(
    ...
    internal fun runStartStage(): Boolean {
        ...
        internalState.basicPlayerOrder.forEachIndexed { idx, it ->
            ...
            it.currentPot = when (it.role) {
                PlayerRole.SMALL_BLIND -> lobby.sharedData.gameStateValues.smallBlind
                PlayerRole.BIG_BLIND   -> lobby.sharedData.gameStateValues.bigBlind
                else                   -> 0
            }
        }
    ...
    internal fun runBiddingStage(hint: Boolean, answer: Boolean): Boolean {
        ...
        while (isBiddingComplete().not()) {
            currentPlayerIdx = (currentPlayerIdx + 1).coerceIn(internalState.basicPlayerOrder.indices)
    ...
    private fun buildQuestionQuery(): QuestionQuery {
        return QuestionQuery(
            1,
            lobby.sharedData.settings.allowedLanguages,
            lobby.sharedData.settings.allowedCategories,
            lobby.sharedData.settings.allowedDifficulties
        )
    }
